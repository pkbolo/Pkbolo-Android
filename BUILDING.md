...
Pkbolo uses [Gradle](http://gradle.org) to build the project and to maintain
...
The following steps should help you (re)build Pkbolo from the command line.

1. Checkout the Pkbolo-Android project source with the command:

        git clone https://github.com/Pkboloapp/Pkbolo-Android.git
...
```bash
pngs_from_svg.py ic_audio.svg /path/to/Pkbolo/res/ 150 --color #000 --opacity 0.54 --suffix _light
pngs_from_svg.py ic_audio.svg /path/to/Pkbolo/res/ 150 --color #fff --opacity 1.00 --suffix _light
```
...
6. Paste the URL for the Pkbolo-Android project when prompted (https://github.com/Pkboloapp/Pkbolo-Android.git).
