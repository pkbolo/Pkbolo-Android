...
import com.pkbolo.securesms.conversation.ConversationItem;
...
import com.pkbolo.securesms.database.GroupReceiptDatabase;
...
import com.pkbolo.securesms.util.DateUtils;
import com.pkbolo.securesms.util.DynamicDarkActionBarTheme;
import com.pkbolo.securesms.util.DynamicLanguage;
import com.pkbolo.securesms.util.DynamicTheme;
import com.pkbolo.securesms.util.ExpirationUtil;
import com.pkbolo.securesms.util.Util;
import com.pkbolo.securesms.database.loaders.MessageDetailsLoader;
...
public class MessageDetailsActivity extends PassphraseRequiredActionBarActivity implements LoaderCallbacks<Cursor> {
  ...
  private void updateRecipients(MessageRecord messageRecord, Recipient recipient, List<MessageDetailsRecipientAdapter.RecipientDeliveryStatus> recipients) {
    final int toFromRes;
  ...
  private class MessageRecipientAsyncTask extends AsyncTask<Void,Void,List<MessageDetailsRecipientAdapter.RecipientDeliveryStatus>> {
    ...
    @Override
    public List<MessageDetailsRecipientAdapter.RecipientDeliveryStatus> doInBackground(Void... voids) {
      ...
      List<MessageDetailsRecipientAdapter.RecipientDeliveryStatus> recipients = new LinkedList<>();

      if (!messageRecord.getRecipient().isGroup()) {
        recipients.add(new MessageDetailsRecipientAdapter.RecipientDeliveryStatus(messageRecord.getRecipient(), getStatusFor(messageRecord.getDeliveryReceiptCount(), messageRecord.getReadReceiptCount(), messageRecord.isPending()), messageRecord.isUnidentified(), -1));
      } else {
        ...
        if (receiptInfoList.isEmpty()) {
          ...
          for (Recipient recipient : group) {
            recipients.add(new MessageDetailsRecipientAdapter.RecipientDeliveryStatus(recipient, MessageDetailsRecipientAdapter.RecipientDeliveryStatus.Status.UNKNOWN, false, -1));
          }
        } else {
          for (GroupReceiptDatabase.GroupReceiptInfo info : receiptInfoList) {
            recipients.add(new MessageDetailsRecipientAdapter.RecipientDeliveryStatus(Recipient.resolved(info.getRecipientId()),
    ...
    @Override
    public void onPostExecute(List<MessageDetailsRecipientAdapter.RecipientDeliveryStatus> recipients) {
      if (getContext() == null) {
    ...
    private MessageDetailsRecipientAdapter.RecipientDeliveryStatus.Status getStatusFor(int deliveryReceiptCount, int readReceiptCount, boolean pending) {
      if      (readReceiptCount > 0)     return MessageDetailsRecipientAdapter.RecipientDeliveryStatus.Status.READ;
      else if (deliveryReceiptCount > 0) return MessageDetailsRecipientAdapter.RecipientDeliveryStatus.Status.DELIVERED;
      else if (!pending)                 return MessageDetailsRecipientAdapter.RecipientDeliveryStatus.Status.SENT;
      else                               return MessageDetailsRecipientAdapter.RecipientDeliveryStatus.Status.PENDING;
    }

    private MessageDetailsRecipientAdapter.RecipientDeliveryStatus.Status getStatusFor(int groupStatus, boolean pending, boolean failed) {
      if      (groupStatus == GroupReceiptDatabase.STATUS_READ)                    return MessageDetailsRecipientAdapter.RecipientDeliveryStatus.Status.READ;
      else if (groupStatus == GroupReceiptDatabase.STATUS_DELIVERED)               return MessageDetailsRecipientAdapter.RecipientDeliveryStatus.Status.DELIVERED;
      else if (groupStatus == GroupReceiptDatabase.STATUS_UNDELIVERED && failed)   return MessageDetailsRecipientAdapter.RecipientDeliveryStatus.Status.UNKNOWN;
      else if (groupStatus == GroupReceiptDatabase.STATUS_UNDELIVERED && !pending) return MessageDetailsRecipientAdapter.RecipientDeliveryStatus.Status.SENT;
      else if (groupStatus == GroupReceiptDatabase.STATUS_UNDELIVERED)             return MessageDetailsRecipientAdapter.RecipientDeliveryStatus.Status.PENDING;
      else if (groupStatus == GroupReceiptDatabase.STATUS_UNKNOWN)                 return MessageDetailsRecipientAdapter.RecipientDeliveryStatus.Status.UNKNOWN;
