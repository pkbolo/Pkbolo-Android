...
import com.pkbolo.securesms.color.MaterialColors;
import com.pkbolo.securesms.components.SwitchPreferenceCompat;
import com.pkbolo.securesms.components.ThreadPhotoRailView;
...
import com.pkbolo.securesms.contacts.avatars.ResourceContactPhoto;
import com.pkbolo.securesms.database.loaders.RecipientMediaLoader;
...
import com.pkbolo.securesms.preferences.CorrectedPreferenceFragment;
import com.pkbolo.securesms.preferences.widgets.ColorPickerPreference;
import com.pkbolo.securesms.preferences.widgets.ContactPreference;
  ...
  public static class RecipientPreferenceFragment extends CorrectedPreferenceFragment {
        ...
        if (recipient.get().getNotificationChannel() != null) {
          ...
          new AsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(Void... voids) {
              ...
              db.setMessageVibrate(recipient.getId(), NotificationChannels.getMessageVibrate(context, recipient.get()) ? RecipientDatabase.VibrateState.ENABLED : RecipientDatabase.VibrateState.DISABLED);
    ...
    private void setSummaries(Recipient recipient) {
      ...
      if (recipient.isGroup()) {
        ...
        IdentityUtil.getRemoteIdentityKey(getActivity(), recipient).addListener(new ListenableFuture.Listener<Optional<IdentityDatabase.IdentityRecord>>() {
          @Override
    ...
    private @NonNull Pair<String, Integer> getVibrateSummary(@NonNull Context context, @NonNull RecipientDatabase.VibrateState vibrateState) {
      if (vibrateState == RecipientDatabase.VibrateState.DEFAULT) {
        return new Pair<>(context.getString(R.string.preferences__default), 0);
      } else if (vibrateState == RecipientDatabase.VibrateState.ENABLED) {
    ...
    private class VibrateChangeListener implements Preference.OnPreferenceChangeListener {
      ...
      public boolean onPreferenceChange(Preference preference, Object newValue) {
        ...
        final RecipientDatabase.VibrateState vibrateState = RecipientDatabase.VibrateState.fromId(value);
        final Context      context      = preference.getContext();
