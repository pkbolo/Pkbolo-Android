...
import com.pkbolo.securesms.groups.GroupMessageProcessor;
import com.pkbolo.securesms.logging.Log;
import com.pkbolo.securesms.service.WebRtcCallService;
...
import com.pkbolo.securesms.util.Hex;
...
import com.pkbolo.securesms.attachments.PointerAttachment;
...
import com.pkbolo.securesms.contactshare.ContactModelMapper;
import com.pkbolo.securesms.crypto.storage.TextSecureSessionStore;
...
import com.pkbolo.securesms.sms.IncomingEncryptedMessage;
import com.pkbolo.securesms.sms.IncomingEndSessionMessage;
...
//    }
                                 ...
                                 @NonNull SignalServiceReceiptMessage message)
  {
    if (TextSecurePreferences.isReadReceiptsEnabled(context)) {
      for (long timestamp : message.getTimestamps()) {
        ...
        DatabaseFactory.getMmsSmsDatabase(context)
                       .incrementReadReceiptCount(new MessagingDatabase.SyncMessageId(Recipient.externalPush(context, content.getSender()).getId(), timestamp), content.getTimestamp());
      }
    }
  }
  ...
  private Optional<MessagingDatabase.InsertResult> insertPlaceholder(@NonNull String sender, int senderDevice, long timestamp, Optional<String> groupId) {
    SmsDatabase         database    = DatabaseFactory.getSmsDatabase(context);
